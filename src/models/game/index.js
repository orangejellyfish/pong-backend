...
class Game {
  ...
  #moveBall() {
    ...
    if (collides(
      ...
      const angle = this.#angle(ball.y - leftPaddle.y);
      ...
      const angle = this.#angle((ball.y + BALL_HEIGHT) / 2) - ((rightPaddle.y + PADDLE_HEIGHT) / 2);
      ball.dy = Math.sin(angle) * BALL_SPEED;
      ball.dx *= -1;
      result.bounce = 1;
  ...
  #angle(paddleStrike) {
    const calcAngle = (Math.PI / 4) * (paddleStrike / PADDLE_HEIGHT) - (Math.PI / 8);
    return calcAngle;
  }
